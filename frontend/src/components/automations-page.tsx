/**
 * FILE: frontend/src/components/automations-page.tsx
 *
 * Componente React de la página de automatizaciones del dashboard.
 * Pertenece al proyecto frontend (Next.js / React) — NO es parte del backend.
 *
 * Requiere los siguientes módulos del proyecto frontend para funcionar:
 *   - @agency/backend: tipos compartidos (Automation)
 *   - @/lib/hooks: hook useVoiceAutomationCenter() que consume GET /api/voice
 *   - @/components/ui: componentes UI (Badge, Button, Card, Input)
 *   - lucide-react: iconos
 *
 * Responsabilidades:
 *   - Mostrar cards de automatizaciones existentes con su estado
 *   - Panel de control del call bot: stats de agentes, flows y jobs
 *   - Lista de call jobs recientes con estado y outcome
 *   - Formulario para crear agentes de voz (nombre + número de salida)
 *   - Formulario para crear call flows (nombre, objetivo, prompt, teclas)
 *   - Formulario para disparar llamadas de prueba seleccionando agente + flow + contacto
 *
 * Comunicación con el backend:
 *   POST /api/voice/agents  → crear agente
 *   POST /api/voice/flows   → crear flow
 *   POST /api/voice/jobs    → disparar llamada
 *   GET  /api/voice         → data del hook (snapshot completo)
 */
